...
export class DieState {
    ...
    public readonly lines: LineMeasurement[] = [];
    ...
    public addLine(lineMeasure: LineMeasurement) {
        this.lines.push(lineMeasure);
        this.updatePolygon();

        lineMeasure.onLengthChange = (oldPoints: Konva.Vector2d, newPoints: Konva.Vector2d) => this.onLengthChange(lineMeasure, oldPoints, newPoints);
    }

    public remove(node?: Konva.Shape | Konva.Stage | Konva.Node) {
        if (!node) return;

        this.lines.find(l => l.group._id == node._id || l.line._id == node._id || l.text.text._id == node._id)?.destroy();
        this.updatePolygon();
    }
    ...
    private onLengthChange(lineMeasure: LineMeasurement, oldPoints: Konva.Vector2d, newPoints: Konva.Vector2d) {
        ...
        const attachedLine = this.findLinesWithEndpoint(oldPoints).filter(l => l.line._id != line._id)[0];
    ...
    public findAttachedLines(line: Konva.Line): Konva.Line[] {
        ...
        for (const mLine of this.lines) {
            const points = KonvaUtils.lineToCoords(mLine.line);
